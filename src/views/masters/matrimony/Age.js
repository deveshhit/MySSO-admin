...
const Age = () => {
    ...
    const [addagetype, setAgetype] = useState('');



    useEffect(() => {
        getAgeList(page, perPage)
    }, [])

    const getAgeList = async (page, perPage) => {
        try {
            const requestOption = {
                method: "GET",
                headers: {
            ...
            await fetch(`${API_URL}/api/ageList?page=${page}&limit=${perPage}`, requestOption)
    ...
    const handlePageClick = (e) => {
        ...
        getAgeList(selectedPage, perPage);
    };
    ...
    const getdetails = async (id) => {
        try {
            ...
            await fetch(`${API_URL}/api/getAgedetailsbyid?id=${id}`, requestOption)
                .then((response) => response.json())
                .then((data) => {
                    if (data.status == 200) {
                        setType(data.data.age)
                    } else {
                        console.log(data.message)
                    }
                })
    ...
    const addusertypedata = async (e) => {
        e.preventDefault();
        console.log("addusertype>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", addagetype)
        try {
            ...
            const requestOption = {
                ...
                body: JSON.stringify({ age: addagetype }),
            }
            await fetch(`${API_URL}/api/addAge`, requestOption)
                .then((response) => response.json())
                .then((data) => {
                    if (data.status == 200) {
                        swal(data.message, {
    ...
    const updateusertype = async (e) => {
        ...
        try {
            ...
            await fetch(`${API_URL}/api/ageUpdate`, {
                method: 'put',
                body: JSON.stringify({ id: id, age: type }),
                headers: {
                    'Content-Type': 'application/json',
                },
            })
    ...
    const updatestatus = (id, status) => {
        try {
            ...
            fetch(`${API_URL}/api/ageUpdate`, requestOption)
                .then(response => response.json())
                .then(data => {
                    console.log(data)
                    if (data.status === 200) {
                                ...
                                <div className="card-header">
                                    <div className="row">
                                        <div className="col-md-6">
                                            <h4 className="card-title">Age List</h4>
                                        </div>
                                ...
                                <div className="card">
                                    <div className="card-body">
                                        <form action="/" method="POST">
                                            <div className="row">
                                                <div className="col-md-6">
                                                    <label className="form-label"><strong>Age Type</strong></label>
                                                    <input type='text' className="form-control"
                                                        name='usertype'
                                                        placeholder='Enter Age Type'
                                                        defaultValue={type}
