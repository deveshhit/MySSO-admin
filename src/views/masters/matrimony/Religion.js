...
const Religion = () => {
    ...
    const [addreligiontype, setReligiontype] = useState('');



    useEffect(() => {
        getReligionList(page, perPage)
    }, [])

    const getReligionList = async (page, perPage) => {
        try {
            const requestOption = {
                method: "GET",
                headers: {
            ...
            await fetch(`${API_URL}/api/religionList?page=${page}&limit=${perPage}`, requestOption)
    ...
    const handlePageClick = (e) => {
        ...
        getReligionList(selectedPage, perPage);
    };
    ...
    const getdetails = async (id) => {
        try {
            ...
            await fetch(`${API_URL}/api/getReligiondetailsbyid?id=${id}`, requestOption)
                .then((response) => response.json())
                .then((data) => {
                    if (data.status == 200) {
    ...
    const addusertypedata = async (e) => {
        e.preventDefault();
        console.log("addusertype>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", addreligiontype)
        try {
            ...
            const requestOption = {
                ...
                body: JSON.stringify({ type: addreligiontype }),
            }
            await fetch(`${API_URL}/api/addReligion`, requestOption)
                .then((response) => response.json())
                .then((data) => {
                    if (data.status == 200) {
                        swal(data.message, {
    ...
    const updateusertype = async (e) => {
        ...
        try {
            ...
            await fetch(`${API_URL}/api/religionUpdate`, {
                method: 'put',
    ...
    const updatestatus = (id, status) => {
        try {
            ...
            fetch(`${API_URL}/api/religionUpdate`, requestOption)
                .then(response => response.json())
                .then(data => {
                    console.log(data)
                    if (data.status === 200) {
                                        ...
                                        <form action="/" method="POST">
                                            <div className="row">
                                                <div className="col-md-6">
                                                    ...
                                                    <label className="form-label"><strong>Religion type</strong></label>
                                                    <input type='text' className="form-control"
                                                        name='type'
                                                        placeholder='Enter Religion Type'
                                                        defaultValue=""
                                                        required
                                                        autoComplete="off"
                                                        onChange={(e) => setReligiontype(e.target.value)}
                                                    />
                                                </div>
                                            </div>
                                            <div className="row">
